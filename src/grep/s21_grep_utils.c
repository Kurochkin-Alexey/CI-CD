...
void define_f_flag(int f_flag, char ***f_flag_lines, char **f_arg,
                   ...
                   int n_flag, int l_flag, int h_flag, int o_flag, int e_flag) {
  if (f_flag) {
    int *i = 0;
    ...
    for (int j = optind + 1; j < argc; j++) {
      ...
      if (file != NULL) {
        ...
        finder(file, argc, i, argv[j], *f_flag_lines, f_cycle_counter,
               ...
               n_flag, l_flag, h_flag, o_flag, file_counter, f_flag, e_flag);
        fclose(file);
      }
    }
    optind--;
...
void finder(FILE *file, int argc, int *cycle, char *filename,
            ...
            int file_counter, int f_flag, int e_flag) {
  char *buffer = NULL;
  ...
  if (!v_flag && !o_flag && !f_flag) {
    ...
    if ((reti = regexec(&regex, buffer + offset, 1, &match, 0)) == 0) {
      ...
      if (file_counter && !h_flag && !c_flag && !l_flag) {
        fprintf(stdout, "%s%s", filename, colon);
      }
      if (n_flag && !c_flag && !l_flag) {
        fprintf(stdout, "%d%s", *line_number, colon);
      }
    }
    ...
    if (f_flag && !o_flag) {
      for (int i = 0; i < num_lines; i++) {
        while ((next_match = (i_flag ? strcasestr(match1, f_flag_lines[i])
          ...
          i_flag ? fprintf(stdout, "%.*s", (int)strlen(f_flag_lines[i]),
                           next_match)
                 : fprintf(stdout, "%.*s", (int)strlen(f_flag_lines[i]),
                           next_match);
    ...
    if (!i_flag) {
      ...
      *reti = regexec(&regex, tmp, 0, NULL, 0);
      if (*reti != 0) {
        ...
        if (new_buffer != NULL) {
          if (file_counter && !h_flag) {
            sprintf(new_buffer, "%s%s", filename, colon);
          }
...
void process_nv_flag(int i_flag, int v_flag, int c_flag, char *required_data,
  ...
  if (v_flag && !i_flag) {
    ...
    if (reti != 0) {
      ...
      if (new_buffer != NULL) {
        if (file_counter && !h_flag) {
          sprintf(new_buffer, "%s%s%d%s", filename, colon, *line_number, colon);
        }
        if (file_counter && h_flag) {
          sprintf(new_buffer, "%d%s", *line_number, colon);
        }
        if (!file_counter) {
          sprintf(new_buffer, "%d%s", *line_number, colon);
        }
    ...
    *buffer[0] = '\0';
    ...
    while (regexec(&regex, tmp + offset, 1, &match, 0) == 0) {
      ...
      if (new_buffer != NULL) {
        ...
        offset += match.rm_eo;
        // Добавляем символ новой строки, если это не последний результат
        ...
        if (found_str != NULL) {
          ...
          if (new_buffer != NULL) {
            ...
            *buffer = new_buffer;
            (*smth_found) = 1;
            found = 1;
            tmp_pointer = found_str + strlen(f_flag_lines[i]);
            break;
          }
        }
      }
